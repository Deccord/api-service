...
export default function NetworkSupplyPage() {
  ...
  const curlRequest = `curl ${config.web.apiUrl}/api/getsupply`
  const fetchRequest = `fetch("${config.web.apiUrl}/api/getsupply")
  ...
  const handleTestApi = async () => {
    setLoading(true)
    try {
      const response = await fetch(`${config.web.apiUrl}/api/getsupply`)
  ...
  return (
    <div className="container mx-auto py-10">
      <div className="max-w-4xl">
        <h1 className="text-3xl font-bold mb-4">Network Supply</h1>
        ...
        <div className="space-y-8">
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Get Network Supply</CardTitle>
              <CardDescription>
